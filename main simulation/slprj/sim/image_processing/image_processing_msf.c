...
#define S_FUNCTION_NAME image_processing_msf
#endif
...
mr_image_processing_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S
, 1 ) ; ssSetParameterTuningCompliance ( S , true ) ; ssSetNumRWork ( S , 0 )
...
mr_image_processing_MdlInfoRegFcn ( S , "image_processing" , & retVal ) ; if
( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( hdsh1qm120k ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { hdsh1qm120k * dw = (
hdsh1qm120k * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } n14buihm2x
...
hdsh1qm120k * dw = ( hdsh1qm120k * ) ssGetDWork ( S , 0 ) ; owbpagqi5h ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_image_processing_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_image_processing_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss
...
mr_image_processing_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss
