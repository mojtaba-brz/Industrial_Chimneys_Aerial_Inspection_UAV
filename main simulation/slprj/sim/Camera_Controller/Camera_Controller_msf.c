...
#define S_FUNCTION_NAME Camera_Controller_msf
#endif
...
mr_Camera_Controller_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S
...
mr_Camera_Controller_MdlInfoRegFcn ( S , "Camera_Controller" , & retVal ) ;
if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( aciultibone ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { aciultibone * dw = (
aciultibone * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } gva3e55kcj
...
aciultibone * dw = ( aciultibone * ) ssGetDWork ( S , 0 ) ; itwqwzhf3m ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_Camera_Controller_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_Camera_Controller_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber (
...
mr_Camera_Controller_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber (
