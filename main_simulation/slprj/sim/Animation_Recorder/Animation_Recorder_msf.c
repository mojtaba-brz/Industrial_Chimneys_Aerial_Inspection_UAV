...
#define S_FUNCTION_NAME Animation_Recorder_msf
#endif
...
mr_Animation_Recorder_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes (
S , 1 ) ; ssSetParameterTuningCompliance ( S , true ) ; ssSetNumRWork ( S , 0
...
mr_Animation_Recorder_MdlInfoRegFcn ( S , "Animation_Recorder" , & retVal ) ;
if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( l1cqubzln1i ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { l1cqubzln1i * dw = (
l1cqubzln1i * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } c1iynsi42k
( S , 0 , & ( dw -> rtm ) , sysRanPtr , sysTid , ( NULL ) , ( NULL ) , 0 , -
...
l1cqubzln1i * dw = ( l1cqubzln1i * ) ssGetDWork ( S , 0 ) ; pedve21wtt ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_Animation_Recorder_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_Animation_Recorder_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber (
...
mr_Animation_Recorder_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber (
