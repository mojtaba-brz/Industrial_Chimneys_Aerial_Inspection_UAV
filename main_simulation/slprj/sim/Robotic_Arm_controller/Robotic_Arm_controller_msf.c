...
#define S_FUNCTION_NAME Robotic_Arm_controller_msf
#endif
...
mr_Robotic_Arm_controller_RegisterSimStateChecksum ( S ) ;
ssSetNumSampleTimes ( S , 1 ) ; ssSetParameterTuningCompliance ( S , true ) ;
...
mr_Robotic_Arm_controller_MdlInfoRegFcn ( S , "Robotic_Arm_controller" , &
retVal ) ; if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( bb3sjqi1nwl ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { bb3sjqi1nwl * dw = (
bb3sjqi1nwl * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } lxn2x0flht
...
bb3sjqi1nwl * dw = ( bb3sjqi1nwl * ) ssGetDWork ( S , 0 ) ; ka1eidyyuy ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_Robotic_Arm_controller_GetDWork ( ssGetDWork ( S , 0 ) ) ;
...
mr_Robotic_Arm_controller_SetDWork ( ssGetDWork ( S , 0 ) ,
