...
#define S_FUNCTION_NAME UAV_Controller_msf
#endif
...
mr_UAV_Controller_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S ,
...
mr_UAV_Controller_MdlInfoRegFcn ( S , "UAV_Controller" , & retVal ) ; if ( !
retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( ievhojrtmvi ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { ievhojrtmvi * dw = (
ievhojrtmvi * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } pi4tqftdo3
...
ievhojrtmvi * dw = ( ievhojrtmvi * ) ssGetDWork ( S , 0 ) ; juvqgef4na ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_UAV_Controller_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_UAV_Controller_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss ,
...
mr_UAV_Controller_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss ,
