...
#define S_FUNCTION_NAME Camera_blur_msf
#endif
...
mr_Camera_blur_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S , 1 )
...
mr_Camera_blur_MdlInfoRegFcn ( S , "Camera_blur" , & retVal ) ; if ( ! retVal
) return ; }
#endif
#if SS_SFCN_FOR_SIM
if ( ssSetNumDWork ( S , 1 ) ) { int mdlrefDWTypeId ; ssRegMdlRefDWorkType (
S , & mdlrefDWTypeId ) ; if ( mdlrefDWTypeId == INVALID_DTYPE_ID ) return ;
if ( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( mgtqaambxbn ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { mgtqaambxbn * dw = (
mgtqaambxbn * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int
sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid
( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } popg1ixb0u
...
mgtqaambxbn * dw = ( mgtqaambxbn * ) ssGetDWork ( S , 0 ) ; mcdksfdgay ( & (
dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_Camera_blur_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_Camera_blur_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss , 0
...
mr_Camera_blur_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss , 0
